                    ...
                    <Row className='detail-body'>
                        ...
                        <Col md={6} xs={6}>
                            ...
                            <RefTreeTableWithInput
                                ...
                                multiple={false}
                                refModelUrl={{
                                    treeUrl: '/bankLoan/common-ref/blobRefTree',
                        ...
                        <Col md={6} xs={6}>
                            ...
                            <RefMultipleTableWithInput
                                ...
                                matchUrl='/pap_basedoc/common-ref/matchPKRefJSON' // 查询
                        ...
                        <Col md={6} xs={6}>
                            ...
                            <RefMultipleTableWithInput
                                placeholder="请选择负责销售员" // 参照input placeholder
                                title='请选择负责销售员'  // 参照弹框 title
                                backdrop={true}  //弹出层是否有模态层，true 显示，false 不显示
                                multiple={false} // 单选
                                strictMode={true}  // 严格模式
                                miniSearch={true} // 展示简单搜索
                                ...
                                displayField={"{name}"} //input中显示的内容的格式和过滤列表显示的内容格式。
                                param={{//url请求参数
                                    refCode: 'xwl_appUser' // refcode  后端给
                                }}
                                refModelUrl={{
                                    tableBodyUrl: '/bankLoan/common-ref/blobRefTreeGrid',//表体请求
                                    refInfo: '/bankLoan/common-ref/refInfo',//表头请求
                                ...
                                onSave={(values) => {  // 确定事件回调
                                    const idArray = values.map(v => v.id); // 获取选中行的数据id
                                    self.setState({
                                        refKeyArrayresponsibleSaler: idArray, // 将选中的id 更新的组件的 state 中
                                    })
                                }}

                                {...getFieldProps('responsibleSaler', {
                                    initialValue: `{"refname":"${responsibleSalerName}","refpk":"${responsibleSaler}"}`, // 参照初始化值
                                    rules: [{
                                        message: '请输入内容', //  参照验证失败错误提示
                                        pattern: /[^{"refname":"","refpk":""}]/   // 参照验证规则
                                    }]
                            ...
                            <span className='error'>{getFieldError('responsibleSaler')}</span>
                        </Col>
                        <Col md={6} xs={6}>
                            <Label>
                            ...
                            <RefMultipleTableWithInput
                                ...
                                placeholder="请选择销售车型" // 参照input placeholder
                                title='请选择销售车型'  // 参照弹框 title
                                backdrop={true}  //弹出层是否有模态层，true 显示，false 不显示
                                multiple={false} // 单选
                                strictMode={true}  // 严格模式
                                miniSearch={true} // 展示简单搜索
                                ...
                                param={{//url请求参数
                                    refCode: 'xwl_vehicleParams' // refcode  后端给
                                }}
                                refModelUrl={{
                                    tableBodyUrl: '/bankLoan/common-ref/blobRefTreeGrid',//表体请求
                                    refInfo: '/bankLoan/common-ref/refInfo',//表头请求
                                ...
                                onSave={(values) => {  // 确定事件回调
                                    const idArray = values.map(v => v.id); // 获取选中行的数据id
                                    self.setState({
                                        refKeyArrayfile_number: idArray, // 将选中的id 更新的组件的 state 中
                                    })
                                }}

                                {...getFieldProps('file_number', {
                                    initialValue: `{"refname":"${file_number_value}","refpk":"${file_number}"}`, // 参照初始化值
                                    rules: [{
                                        message: '请输入内容', //  参照验证失败错误提示
                                        pattern: /[^{"refname":"","refpk":""}]/   // 参照验证规则
                                    }]
